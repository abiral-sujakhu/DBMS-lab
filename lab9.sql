...
-- 2
 create view emp_view_IT as select eid,ename,age from employee where dept='IT';
 select * from emp_view_IT;
 
 -- 3
 CREATE VIEW vw_employee_company_details AS
SELECT e.eid, e.ename,c.companyname,c.city,w.salary FROM employee e
JOIN works w   ON e.eid = w.eid
JOIN company c ON w.compid = c.compid;

select * from vw_employee_company_details;

-- 4
 create view dept_NAME_AVG_SAL1 as select e.dept,avg(w.salary) as AVG_sal from employee e
 join works w on e.eid=w.eid
 group by e.dept;

 select * from dept_NAME_AVG_SAL1;

-- 5 
select ename from emp_view_IT where age>25;
-- 6
create view eid_sal_comp as select eid,salary,compid from works ;
 select * from eid_sal_comp;
 
 update eid_sal_comp set salary=salary+3000 where compid=102;
 
 --  7
 create view com_city as select compid,companyname,city from company;
 select * from com_city;
 
 update com_city set city='Kavre' where companyname='F1Soft';
 
 -- 8	
 insert into com_city (compid,companyname,city) values(106,'Vrit','Bhaktapur');
 
 -- 9 
 insert into emp_view (eid,ename,age,dept) values(51,'emp51',20,'IT');
 
 -- 10
 delete from emp_view where age<25 and dept='IT';
 
 -- 11
 delete from com_city where compid=106;
 
 -- 12
 drop view emp_view,com_city;
